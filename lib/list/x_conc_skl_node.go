...
type _xConcSklNode[K infra.OrderedKey, V any] struct {
	// Type is K
	key unsafe.Pointer // unsafe pointer size 8, 2 bytes
	// Type is *xNode[V]
	root unsafe.Pointer // unsafe pointer size 8, 2 bytes; used to store value
	// Type is segmentMutex
	// mu       unsafe.Pointer // unsafe pointer size 8, 2 bytes
	// Arena offset array
	indices [sklMaxLevel]uint32 // size 128, 32 bytes
	flagBits uint64         // size 8, 2 byte
	count    uint32         // size 4, 1 byte
	level    uint32         // size 4, 1 byte
}

func (node *_xConcSklNode[K, V]) atomicSet(bits uint64) {
	for {
...
func (node *_xConcSklNode[K, V]) set(bits uint64) {
	node.flagBits = node.flagBits | bits
}

// Bit flag set from 1 to 0.
func (node *_xConcSklNode[K, V]) atomicUnset(bits uint64) {
	for {
		old := atomic.LoadUint64(&node.flagBits)
		check := old & bits
		if check != 0 {
...
func (node *_xConcSklNode[K, V]) atomicIsSet(bit uint64) bool {
	return (atomic.LoadUint64(&node.flagBits) & bit) != 0
}

func (node *_xConcSklNode[K, V]) atomicAreEqual(bits, expect uint64) bool {
...
func (node *_xConcSklNode[K, V]) atomicLoadBits(bits uint64) uint64 {
	if ibits.HammingWeightBySWARV2[uint64](bits) <= 1 {
		panic("it is not a multi-bits")
	}
	n := 0
...
func (node *_xConcSklNode[K, V]) setBitsAs(bits, target uint64) {
	if ibits.HammingWeightBySWARV2[uint64](bits) <= 1 {
		panic("it is not a multi-bits")
	}
	n := 0
...
func (node *_xConcSklNode[K, V]) atomicSetBitsAs(bits, target uint64) {
	if ibits.HammingWeightBySWARV2[uint64](bits) <= 1 {
		panic("it is not a multi-bits")
	}
	n := 0
...
func (node *_xConcSklNode[K, V]) isSet(bit uint64) bool {
	return (node.flagBits & bit) != 0
}

func (node *_xConcSklNode[K, V]) loadBits(bits uint64) uint64 {
	if ibits.HammingWeightBySWARV2[uint64](bits) <= 1 {
		panic("it is not a multi-bits")
	}
	n := 0
...
func (node *_xConcSklNode[K, V]) areEqual(bits, expect uint64) bool {
